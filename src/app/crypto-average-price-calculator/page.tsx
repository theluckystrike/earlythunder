...
const spec: CalculatorPageSpec = {
  ...
  sections: [
    { heading: "Why a simple average is wrong", paragraphs: ["Before fees, adding two prices and dividing by two is only correct when both purchases contain exactly the same number of units. A new fee changes even that equal-size result. If one buy is larger, its execution price must also carry more weight. The modeled average therefore comes from total cash cost divided by total units.", "This distinction matters when averaging down or adding after a rally. A small second order barely moves a large existing position, even if its price is far from the original entry."] },
